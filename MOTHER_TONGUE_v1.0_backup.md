...
Read: *Human acts, implies future arrives.*
Form: `SUBJECT VERB ⇒ RESULT`
...
Read: *Mind is-inside Void.*
Scoped groups are opaque from outside; κ resets within `【 】` for local counting but the clause-level κ includes all distinct constraints.

### 3) Chaining

κ=4  (two clauses joined by connective)

```mt
【 力 】 入 【 ◯ 】 ∧ 不 【 破 】
```

Read: *Force enters Potential AND not Destruction.*
`∧` joins complete clauses. `∨` is disjunctive. `و` joins loose enumerations where order is irrelevant.

### 4) Function Application

κ=3  (abstraction + application + output)

```mt
λ 用 ⇒ 示
```

Read: *Function applied, implies output.*
`λ` declares abstraction; `用` applies it. The result flows rightward.

### 5) Possessive / Membership
...
Read: *Totality's Source.*
`之` binds right to left: `A 之 B` = *A's B*.
...
Read: *Break the loop.* (terminate recursion)
`⭮` applied alone means *repeat*. Preceded by `破`, the cycle is broken.
...
Read: *Not good. Sever matter.*
`不` is passive negation (modifier). `絕` is active severance (destroys a relation).
